...
# from Py_FS.wrapper.nat   ure_inspired._utilities import Solution, Data, initialize, sort_agents, display, compute_fitness, Conv_plot
...
def foldvalpoly(df):
    ...
    for train_ix, test_ix in kfold.split(X, y):
        ...
        train_X, train_y = over.sample(train_X, train_y)
        clf = DecisionTreeClassifier().fit(train_X, train_y)
        test_y_predicted = clf.predict(test_X)
...
if __name__ == '__main__':
    filepath = '/Users/emily/Desktop/ISOLET.csv'
    df = readData(filepath)
    meanDe = foldvalipf(df)
    # meanDe = foldvalpoly(df)
    # meanDe = foldvalprowsyn(df)
    print('準確率', meanDe)
