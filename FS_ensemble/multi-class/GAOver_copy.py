...
def ElbowCenterGenerate(train, ratio, method, path):
    ...
    origincount = classCount
    print("要產生的資料數", origincount)
    countfor = math.floor(
        int(classCount)*ratio)  # 要產生多少數據  無條件捨去
...
def calculatethreemethod(train, approach, sheetName, path, test_X, test_y):
    ...
    for index in range(len(f)):
        ...
        train_X, train_y = mergedata.iloc[:, :-1], mergedata.iloc[:, -1]
        ...
        accuracyDe.append(roc_auc_score(
            test_y, test_y_predicted, multi_class="ovr"))
    print(accuracyDe)
...
def main(datapath):
    ...
    for train_ix, test_ix in kfold.split(X, y):
        ...
        temp = np.array(calculatethreemethod(
            dfafterselect, 'approach', 'firstwrappe', '', test_X, test_y))
...
if __name__ == '__main__':
    ...
    token = 'bCpvlorCZoJPxA2thceajfMrdgwZhumsVTYMMOGXg4q'
    lineNotifyMessage(
        token, 'svm Urban_land_cover GA + oversampling ensemble')
    meanDe = main(
        '/Users/emily/Desktop/Research/oversampling_python/data/multi-class/Urban_land_cover/all.csv')
