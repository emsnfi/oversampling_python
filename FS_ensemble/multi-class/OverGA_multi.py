...
def synth(finaldata, output, method):
    ...
    if method is 'poly':  # "poly" in method:
        ...
        over = sv.MulticlassOversampling(sv.polynom_fit_SMOTE())
    elif method is 'prow':  # "proW" in method:
        print("pro")
        over = sv.MulticlassOversampling(sv.ProWSyn())
    elif method is 'SMOTEIPF':  # "SMOTEIPF" in method:
        print("smoteipf")
        over = sv.MulticlassOversampling(sv.SMOTE_IPF())
        # over = sv.SMOTE_IPF()
...
def ElbowCenterGenerate(train, ratio, method, path):
    ...
    X_polynom = MinMaxScaler().fit(X_polynom).transform(X_polynom)
    ...
    for i in range(len(classCount)):  # 不同類個別要產生多少數據才能平衡 目前是二分類
        ...
        dtemp = dtemp.iloc[originlen:, :]  # 後來生成的 都是小類
        print('counter', Counter(dtemp.iloc[:, -1]))
        dtemporigin = dtemp  # 存放都是小類的資料 不分 class

        if(countfor > 0):

            dfclass = int(classCount[i][0])
            print('=============要生成的類別', dfclass, '原本數量', classCount[i][1], 'ratio', ratio,
                  '需要的數量', countfor, '===============')
            # dtemp = pd.DataFrame(overpolynom)
            ln = dtemp.columns[-1]
            '''改 因為是 multi class'''
            dtemp = dtemp[dtemp[ln] == dfclass]
            print('新生成的那一 class', dtemp.shape)  # 新生成的那一類
            # Xtest = dtemp[dtemp[ln] == minclass] # 只專注在小類
            print('確認要生成的class', Counter(dtemp.iloc[:, -1]))
            # X = dtemp[dtemp[ln] == minclass].iloc[:,
            #                                       :dtemp.shape[1]-1]  # 選取小類的資料
            X = dtemp.iloc[:, :dtemp.shape[1]-1]  # 該 class X
            # print('X', X.shape)
            # print('Xdata', X)
            # 存放有 index 的 該 class 數值
            # Xorigin = X.rename_axis('index').reset_index()
            X = X.rename_axis('index').reset_index()
            # X.reset_index(inplace=True, drop=True)
            print('Xdatareset\n', X)
            xc = X.iloc[:, 1:]  # 去掉index
            Xcluster = np.array(xc)
            # 計算應該分成幾群
            ...
            # cluster_count = 3  # 只針對 lun_discrete
        ...
        # 計算每個點跟各群中心的距離
            ...
            # 把不同群過濾出來
                ...
                # tempdf 是指那一群的資料
                tempdf = df[df['label'] == ic]  # 該 class 該群
                print('tempdf\n', tempdf)
                tempdf = tempdf.iloc[:, :-1]  # 去掉 label

                print(ic, '類共有幾個data', tempdf.shape[0], 'len', len(tempdf))
            # allCluster.append(df[df['label']==ic])
            # 計算每個點跟群中心的距離
                centeric = np.array(centers[ic])
                # 為了要計算距離，要先把 index column 清掉
                tempforeucal = tempdf.iloc[:, 1:]
                tempforeucal = np.array(tempforeucal.values)
                # print(type(centeric))
                # print(tempdf.shape)
                # tempdata 是 index
                for i in range(tempforeucal.shape[0]):  # 列 也就是幾筆資料 index
                    index = tempdf.iloc[i, 0]  # index 欄位的數值
                    tempindata[index] = distance.euclidean(  # 排序跟center計算出來的距離 tempindata 是一個dict key 存的是原始的index, value存的是距離
                        centeric, tempforeucal[i])
                print('tempindata', tempindata)
            ...
            # print("比例",labelRatio)
                ...
                tempcenterpolynom = tempcenterpolynom + temppolynom  # 該 class
                # tempcenterpolynom.extend(temppolynom)
            tempcenterpolynom = np.array(tempcenterpolynom)
            print('該class', tempcenterpolynom)
            # print('======data', type(tempcenterpolynom), '===========')
            print('templen', len(tempcenterpolynom))  # 該 class 所要數量

            labelclass = np.repeat(dfclass, len(tempcenterpolynom))
            print('labelclass', np.shape(labelclass))
            tempcenterpolynom = np.array(tempcenterpolynom)
            t = np.column_stack([tempcenterpolynom, labelclass])

            centerpolynom.append(t)  # 所有群所要的資料集 # 所有資料集所選到的資料
            # print('class', dfclass, ' ', t)
    print('所有class', centerpolynom)
    alltemp = []
    '''binary class'''
    ...
    '''multi class'''
    for i in range(len(centerpolynom)):  # loop class
        for j in range(len(centerpolynom[i])):
            indexpolynom = int(centerpolynom[i][j][0])
            print(int(indexpolynom))
            alltemp.append(list(overpolynom.iloc[indexpolynom]))
...
def calculatethreemethod(train, approach, sheetName, path, test_X, test_y):
    ...
    for index in range(len(f)):
        ...
        # 把 GA 放到 calculate 內
        dfselect = GAselect(mergedata)
        '''計算 accuracy'''
        train_X, train_y = dfselect.iloc[:, :-1], dfselect.iloc[:, -1]
        print('final ratio', Counter(train_y))
        ...
        accuracyDe.append(roc_auc_score(test_y, pred_prob, multi_class='ovr'))
    print(accuracyDe)
...
def main(datapath):
    ...
    for train_ix, test_ix in kfold.split(X, y):
        ...
        temp = calculatethreemethod(  # 回傳每一份 acc
            traindf, 'approach', 'firstwrappe', '', test_X, test_y)
        # temp = GAselect(mergedata)
    ...
    for i in sumaccu:
        wb = load_workbook(
            '/Users/emily/Desktop/Research/FS_ensemble/multi_class/multi-class_run.xlsx')
        sheet = wb['GA+over']
...
if __name__ == '__main__':
    ...
    token = 'bCpvlorCZoJPxA2thceajfMrdgwZhumsVTYMMOGXg4q'
    # lineNotifyMessage(
    #     token, 'c4.5 robot-failures-lp4 GA + oversampling ensemble')
    meanDe = main(
        '/Users/emily/Desktop/lung_discrete.csv')
