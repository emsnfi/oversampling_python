...
def calculatethreemethod(train, approach, sheetName, path, test_X, test_y):
    ...
    for index in range(len(f)):
        ...
        # clf = svm.SVC(kernel='rbf', C=1).fit(train_X, train_y)  # rbf linear
    ...
    message = '準確率 just oversampling' + accstr
...
if __name__ == '__main__':
    ...
    f = temp
    token = 'bCpvlorCZoJPxA2thceajfMrdgwZhumsVTYMMOGXg4q'
    lineNotifyMessage(token, 'c4.5 emotion just oversampling ensemble')
    meanDe = main(
        '/Users/emily/Desktop/emotions.csv')
