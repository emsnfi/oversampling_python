...
def predictSVM(train, test, allRandomHalf):
    ...
    for index, element in enumerate(train):
        ...
        clf = svm.SVC(kernel='rbf', C=1, gamma='auto')  # 要改成 linear
        clf = clf.fit(finaldata, output)
...
def predictXG(train, test, allRandomHalf):
    mergeRandom = []
    accuracies = []
    for index, element in enumerate(train):
        data = pd.read_excel(element, index_col=0)
...
def predictRF(train, test, allRandomHalf):
    mergeRandom = []
    accuracies = []
    for index, element in enumerate(train):
        data = pd.read_excel(element, index_col=0)
        ...
        forest_fit = forest.fit(finaldata, output)
